...
//search product in invoice
    ...
    public function pos_search()
    {
        ...
        foreach ($result as $row) {
            ...
            $difference = date_diff( $createdAt, $today);
            $diff = $difference->format("%R%a");
            if($diff == 0)
                $diff = 1;
            // $product_price = $row['product_price'] + ($row['daily_feed'] * $diff);
            $product_price = $row['fproduct_price'] + ($row['daily_feed'] * $diff);

            // echo "diff = ".$diff."  ".$product_price;
            $out .= '    <div class="col-3 border mb-1 "><div class="rounded">
                                 <a   id="posp' . $i . '"  class="select_pos_item btn btn-outline-light-blue round"  data-feed="' . $row['daily_feed'] . '" data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($product_price, 0, $this->aauth->get_user()->loc) . '"  data-tax="' . amountFormat_general($row['taxrate']) . '"  data-discount="' . amountFormat_general($row['disrate']) . '"   data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . amountFormat_general($row['qty']) . '" data-unit="' . $row['unit'] . '" data-rprice="' . $row['product_price'] . '" data-serial="' . @$row['serial'] . '">
                                        <img class="round"
                                             src="' . base_url('userfiles/product/' . $row['image']) . '"  style="max-height: 100%;max-width: 100%">
                                        <div class="text-xs-center text">
