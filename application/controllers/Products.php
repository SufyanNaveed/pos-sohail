...
class Products extends CI_Controller
{
    ...
    public function product_list()
    {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = +$prd->qty;
            $date=date_create($prd->created_at);

            $earlier = new DateTime($prd->created_at);
            $later = new DateTime("now");
            $diff = round($later->diff($earlier)->format("%a"));
            
            // $earlier = new DateTime("02-02-2021");
            // $later = new DateTime("now");            
            // echo $diff = $later->diff($earlier)->format("%a");
            

            // $diff = abs($difference->format("%R%a"));
            if($diff == 0)
                $diff = 1;
            $total = $prd->fproduct_price + ($prd->daily_feed * $diff);


            $row[] = date_format($date,"d-m-Y");//. "(".$diff.")";
            $row[] = $prd->c_title;
            
            $row[] = $prd->daily_feed. ' x '.$diff.' Days</span>';
            ...
            $row[] = '<span title="Purchase Price + Daily Fedd">'.amountExchange($total, 0, $this->aauth->get_user()->loc).'</span>';
...
//ini_set('display_errors', 1);
        ...
        if ($catid) {
            $this->products->addnew($catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $v_type, $v_stock, $v_alert, $wdate, $code_type, $w_type, $w_stock, $w_alert, $sub_cat, $brand, $serial, $daily_feed);
        }
    }
    ...
    public function editproduct()
    {
        ...
        $daily_feed = $this->input->post('daily_feed');
        if ($pid) {
            $this->products->edit($pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat, $brand, $vari, $serial, $daily_feed);
        }
    }
